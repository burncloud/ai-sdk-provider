...
describe('doGenerate', () => {
  const server = new JsonTestServer('https://ai.burncloud.com/v1/completions');
  ...
  it('should extract logprobs', async () => {
    ...
    const provider = createBurnCloud({ apiKey: 'test-api-key' });
    ...
    expect(response.logprobs).toStrictEqual(
      mapBurnCloudCompletionLogProbs(TEST_LOGPROBS),
    );
  });
...
describe('doStream', () => {
  const server = new StreamingTestServer(
    'https://ai.burncloud.com/v1/completions',
  );
  ...
  it('should stream text deltas', async () => {
    ...
    expect(await convertReadableStreamToArray(stream)).toStrictEqual([
      ...
      { type: 'text-delta', textDelta: '' },
      {
        type: 'finish',
        finishReason: 'stop',
        logprobs: mapBurnCloudCompletionLogProbs(TEST_LOGPROBS),
